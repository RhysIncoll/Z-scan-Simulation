...
// ─── Log slider ───────────────────────────────────────────────────────────────
// Slider that operates on log10 scale so you can sweep many decades smoothly

function LogSlider({ label, value, min, max, onChange, color }: {
  label: string;
  value: number;      // actual value (e.g. 1e-11)
  min: number;        // log10 min (e.g. -14)
  max: number;        // log10 max (e.g. -8)
  onChange: (v: number) => void;
  color: string;
}) {
  const logVal = value === 0 ? min : Math.log10(Math.abs(value));
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-cyan-300">{value === 0 ? "0" : fmtSci(value)}</span>
      </div>
      <input
        type="range"
...
export default function ZScanDashboard() {
  ...
  const apiBeam = {
    lam: beam.lam, w0: beam.w0, L: beam.L, S: beam.S,
    P_peak, d_det: beam.d_det, tpv_min: beam.tpv_min,
  };
  ...
  // Update a single material's beta or alpha_0 from sliders
  const setMatBeta   = (i: number, v: number) =>
    setMaterials(m => m.map((mat, j) => j === i ? { ...mat, beta: v } : mat));
  const setMatAlpha  = (i: number, v: number) =>
    setMaterials(m => m.map((mat, j) => j === i ? { ...mat, alpha_0: v } : mat));
  const setMatISat   = (i: number, v: number) =>
    setMaterials(m => m.map((mat, j) => j === i ? { ...mat, I_sat: v } : mat));
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 border-r border-slate-800 p-4 flex flex-col gap-4 overflow-y-auto shrink-0">
          ...
          <CollapsibleSection title="Laser Parameters">
            <SciField label="Wavelength λ (m)"     value={beam.lam}       onChange={setBeamField("lam")}       help="e.g. 800e-9 for 800 nm" />
          ...
          <div className="flex-1 overflow-auto p-6">
            ...
            {activeTab === "tz" && (
              <div className="flex flex-col gap-6">
                ...
                {/* ── TPA / SA Sliders ── */}
                {results.length > 0 && (useTPA || useSA) && (
                  <div className="border border-slate-800 rounded-lg p-4 flex flex-col gap-5 bg-slate-900/50">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        Nonlinear Absorption — adjust &amp; re-run
                      </p>
                      <span className="text-xs text-slate-600">Values stored per material</span>
                    </div>

                    {materials.map((mat, i) => (
                      <div key={i} className="flex flex-col gap-3">
                        <div className="flex items-center gap-2">
                          <div className="w-2.5 h-2.5 rounded-full" style={{ background: mat.color }} />
                          <span className="text-xs font-medium text-slate-300">{mat.name}</span>
                        </div>

                        {useTPA && (
                          <div className="pl-4">
                            <LogSlider
                              label="β — TPA coefficient (m/W)"
                              value={mat.beta}
                              min={-14} max={-8}
                              onChange={v => setMatBeta(i, v)}
                              color={mat.color}
                            />
                          </div>
                        )}
                        ...
                        {useSA && (
                          <div className="pl-4 flex flex-col gap-3">
                            <LogSlider
                              label="α₀ — SA linear absorption (m⁻¹)"
                              value={mat.alpha_0}
                              min={0} max={8}
                              onChange={v => setMatAlpha(i, v)}
                              color={mat.color}
                            />
                            <LogSlider
                              label="I_sat — saturation intensity (W/m²)"
                              value={mat.I_sat}
                              min={9} max={14}
                              onChange={v => setMatISat(i, v)}
                              color={mat.color}
                            />
                          </div>
                        )}
                    ...
                    <button
                      onClick={runSimulation} disabled={loading}
                      className="self-start bg-cyan-500 hover:bg-cyan-400 disabled:opacity-40 text-slate-950 font-bold text-xs px-4 py-2 rounded transition-colors"
                    >
                      {loading ? "Running…" : "Re-run with these values"}
                    </button>
                  </div>
                )}
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-800 text-left">
                          {["Material","n₂ (m²/W)","ΔΦ₀ (rad)","β (m/W)","q₀","α₀ (m⁻¹)","I_sat (W/m²)","Tpv","Detectable"].map(h => (
                        ...
                        {results.map((r, i) => {
                          ...
                          return (
                            <tr key={i} className="border-b border-slate-900 hover:bg-slate-900 transition-colors">
                              ...
                              <td className="py-3 pr-6 text-slate-400 font-mono">{mat ? fmtSci(mat.I_sat) : "—"}</td>
